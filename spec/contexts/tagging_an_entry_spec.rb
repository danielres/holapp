...
describe TaggingAnEntry do
  ...
  describe 'tagging a project on skills' do
    ...
    context 'as a guest user' do
      let(:tagger){ User.new }
      it 'does not tag' do
        expect( taggable.tags ).not_to include 'tag1'
        expect( taggable.tags ).not_to include 'tag2'
      end
    end
    context 'as an authorized tagger' do
      ...
      it 'tags the project on skills' do
        expect( taggable.tags ).to include 'tag1'
        expect( taggable.tags ).to include 'tag2'
      end
    end
  end
